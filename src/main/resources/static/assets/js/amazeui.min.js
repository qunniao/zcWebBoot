        ...
        return function(t) {
            ...
            return e.m = t, e.c = i, e.p = "", e(0)
                ...
                function(t, e, i) {
                    "use strict";
                    var n = i(1);
                    if ("undefined" == typeof n) throw new Error("Amaze UI 2.x requires jQuery :-(%n爱上一匹野马，可你的家里没有草原…");
                    var o = n.AMUI || {}, s = n(window),
                        a = window.document,
                        r = n("html");
                    o.VERSION = "2.6.2", o.support = {}, o.support.transition = function() {
                        ...
                        return i.cache[t] || (i.cache[t] = function() {
                            var e = t,
                                ...
                                o = 1,
                                s = ("try { " + (i.variable ? "var " + i.variable + " = this.stash;" : "with (this.stash) { ") + "this.ret += '" + n.replace(/<%/g, "").replace(/%>/g, "").replace(/'(?![^%x11%x13]+?%x13)/g, "%%x27").replace(/^%s*|%s*$/g, "").replace(/%n/g, function() {
                                    return "';%nthis.line = " + ++o + "; this.ret += '%%n"
                                }).replace(/%x11-(.+?)%x13/g, "' + ($1) + '").replace(/%x11=(.+?)%x13/g, "' + this.escapeHTML($1) + '").replace(/%x11(.+?)%x13/g, "'; $1; this.ret += '") + "'; " + (i.variable ? "" : "}") + "return this.ret;} catch (e) { throw 'TemplateError: ' + e + ' (on " + e + "' + ' line ' + this.line + ')'; } //@ sourceURL=" + e + "%n").replace(/this%.ret %+= '';/g, ""),
                                a = new Function(s),
                                r = {
                                    "&": "&amp;",
                                    "<": "&lt;",
                ...
                function(t, e, i) {
                    ...
                    function a(t, e, i) {
                        var n = "DEPRECATED METHOD: " + e + "%n" + i + " AT %n";
                        return function() {
                            var e = new Error("get-stack-trace"),
                                i = e && e.stack ? e.stack.replace(/^[^%(]+?[%n$]/gm, "").replace(/^%s+at%s+/gm, "").replace(/^Object.<anonymous>%s*%(/gm, "{anonymous}()@") : "Unknown Stack Trace",
                    ...
                    function k(t, e) {
                        var i = e.center,
                            n = t.offsetDelta || {}, o = t.prevDelta || {}, s = t.prevInput || {};
                ...
                function(t, e, i) {
                    ...
                    var h, u = /%/ath ( % /)?$/, d = /([%?&]ath=[^&]*$|&ath=[^&]*(&))/; o.intl = {
                            ...
                            for (var p in o.intl) o.intl[p.substr(0, 2)] = o.intl[p]; o.defaults = {
                            ...
                            var m = window.navigator.userAgent, f = window.navigator; s(o, {
                            ...
                            var n = i(2),
                                o = function() {
                                    ...
                                    var h = function() {
                                        ...
                                        u = function() {
                                            function t(t) {
                                                return t.replace(/^-ms-/, "ms-").replace(/-([Ú-z])/gi, function(t, e) {
                        ...
                        function(t, e, i) {
                            ...
                            s.DEFAULTS = {
                                ...
                                searchBox: !1,
                                tpl: '<div class="am-selected am-dropdown <%= dropUp ? %'
                                am - dropdown - up % ': %' % ' %>" id="<%= id %>" data-am-dropdown>  <button type="button" class="am-selected-btn am-btn am-dropdown-toggle">    <span class="am-selected-status am-fl"></span>    <i class="am-selected-icon am-icon-caret-<%= dropUp ? %'
                                up % ' : %'
                                down % ' %>"></i>  </button>  <div class="am-selected-content am-dropdown-content">    <h2 class="am-selected-header"><span class="am-icon-chevron-left">返回</span></h2>   <% if (searchBox) { %>   <div class="am-selected-search">     <input autocomplete="off" class="am-form-field am-input-sm" />   </div>   <% } %>    <ul class="am-selected-list">      <% for (var i = 0; i < options.length; i++) { %>       <% var option = options[i] %>       <% if (option.header) { %>  <li data-group="<%= option.group %>" class="am-selected-list-header">       <%= option.text %></li>       <% } else { %>       <li class="<%= option.classNames%>"          data-index="<%= option.index %>"          data-group="<%= option.group || 0 %>"          data-value="<%= option.value %>" >         <span class="am-selected-text"><%= option.text %></span>         <i class="am-icon-check"></i></li>      <% } %>      <% } %>    </ul>    <div class="am-selected-hint"></div>  </div></div>',
                        ...
                        function(t, e, i) {
                            ...
                            s.DEFAULTS = {
                                ...
                                url: /^(https?|ftp):%/ % /(((([a-zA-Z]|%d|-|%.|_|~|[ -퟿豈-﷏ﷰ-￯])|(%[Ú-f]{2})|[!%$&'%(%)%*%+,;=]|:)*@)?(((%d|[1-9]%d|1%d%d|2[0-4]%d|25[0-5])%.(%d|[1-9]%d|1%d%d|2[0-4]%d|25[0-5])%.(%d|[1-9]%d|1%d%d|2[0-4]%d|25[0-5])%.(%d|[1-9]%d|1%d%d|2[0-4]%d|25[0-5]))|((([a-zA-Z]|%d|[ -퟿豈-﷏ﷰ-￯])|(([a-zA-Z]|%d|[ -퟿豈-﷏ﷰ-￯])([a-zA-Z]|%d|-|%.|_|~|[ -퟿豈-﷏ﷰ-￯])*([a-zA-Z]|%d|[ -퟿豈-﷏ﷰ-￯])))%.)+(([a-zA-Z]|[ -퟿豈-﷏ﷰ-￯])|(([a-zA-Z]|[ -퟿豈-﷏ﷰ-￯])([a-zA-Z]|%d|-|%.|_|~|[ -퟿豈-﷏ﷰ-￯])*([a-zA-Z]|[ -퟿豈-﷏ﷰ-￯])))%.?)(:%d*)?)(%/ ((([a - zA - Z] | % d | - | % . | _ | ~ | [ -퟿豈-﷏ﷰ-￯]) | ( % [Ú-f] {
                                    2
                                }) | [! % $ & '%(%)%*%+,;=]|:|@)+(%/(([a-zA-Z]|%d|-|%.|_|~|[ -퟿豈-﷏ﷰ-￯])|(%[Ú-f]{2})|[!%$&' % ( % ) % * % +, ; = ] | : | @) * ) * ) ? ) ? ( % ? ((([a - zA - Z] | % d | - | % . | _ | ~ | [ -퟿豈-﷏ﷰ-￯]) | ( % [Ú-f] {
                                    2
                                }) | [! % $ & '%(%)%*%+,;=]|:|@)|[-]|%/|%?)*)?(%#((([a-zA-Z]|%d|-|%.|_|~|[ -퟿豈-﷏ﷰ-￯])|(%[Ú-f]{2})|[!%$&' % ( % ) % * % +, ; = ] | : | @) | % /|%?)*)?$/, number: /^-?(?:%d+|%d{1,3}(?:,%d{3})+)?(?:%.%d+)?$/, dateISO: /^%d{4}[%/%-]%d{1,2}[%/%-]%d{1,2}$/, integer: /^-?%d+$/
